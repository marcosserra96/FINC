...
- **Tempo esgotado na atividade:** limite de tempo configurável (padrão 3 min, gestão de fila do evento) — ao zerar, mesmo que o visitante esteja ativamente jogando, leva pra uma tela dedicada de aviso amigável (não é tratado como erro nem como "perdeu"), com retorno automático em alguns segundos ou botão para voltar na hora. Métrica `activity_timeout` registrada. Ver nota sobre "sem pressão de tempo" abaixo.
