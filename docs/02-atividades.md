...
- **Critério de conclusão válida:** encontrar todos os pares sem nenhuma tentativa errada no caminho (100% de acerto nas tentativas). Diferente das outras atividades, aqui não existe um "70% razoável" — o jogo só termina quando todos os pares são encontrados (então `correct` sempre chega ao total), e a única forma de medir acerto de verdade é pelas tentativas erradas ao longo do caminho. Por isso o critério de brinde é binário: ou a pessoa lembra de todos os pares de primeira, ou não ganha.
- **Elegível a brinde:** sim — reativado junto com as demais, mas com critério próprio (ver acima) em vez do "acerto mínimo em %" usado nas outras três atividades.
