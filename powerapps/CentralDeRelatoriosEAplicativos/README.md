...
12. **Filtro de Área: dropdown → botões (chips), a pedido do usuário.** `drpArea` (`ModernDropdown`) foi substituído por `galAreaTabs`, uma `Gallery@2.15.0` horizontal (`Variant: Horizontal`) com rolagem lateral (`ShowScrollbar: true`), alimentada pela mesma coleção `colAreas` de sempre. Cada item é um chip `HtmlViewer` no mesmo estilo visual das abas "Todos/Relatórios/Aplicativos" (fundo escuro quando selecionado, `OnSelect: =Set(varAreaSelecionada, ThisItem.Value)`). A seleção passa a ser guardada em `varAreaSelecionada` (novo, inicializado em `"Todas"` no `OnVisible`) em vez de `drpArea.Selected.Value` — todas as fórmulas que liam esse valor (filtro da galeria principal, chip "Limpar", estado vazio) foram atualizadas.
    - **Sem animação de slide.** Diferente da prévia em HTML, a seleção aqui é instantânea (mesmo padrão que as abas de Tipo já usavam) — uma "pastilha deslizante" animada via `transform`/`transition` dentro de `HtmlViewer` é frágil no Power Apps real e vai contra a própria orientação original de evitar animação sem função e HTML propenso a bugs de renderização.
    - **Rolagem horizontal é esperada aqui** (diferente da rolagem dentro de cards, que continua proibida): como a lista de áreas é dinâmica e pode ter mais opções do que cabem na largura do filtro, os chips rolam lateralmente, um padrão comum e bem estabelecido para filtros em chips.
    - Mantive exatamente a mesma posição/largura que o dropdown antigo ocupava, então não foi preciso realinhar Categoria, Busca ou Limpar.
